...
UpdateMaster = {
    ...
    SavePatient: function () {
        ...
        if (UpdateDetail.ValidateObj()) {

            var patient = UpdateDetail.CreateObject();

            $.ajax({
                async: false,
                type: "PUT",
                contentType: "application/json",
                url: " https://localhost:44314/api/patient/put",
            ...
            window.location.href = 'https://localhost:7098/patient/index'
...
UpdateDetail = {
    Initial: function () {
        UpdateDetail.SetDropDownValue();

        var pathname = window.location.href.toLowerCase();
        var clientUrl = pathname.split('https://localhost:7098/patient/edit/');
        $.ajax({
            async: false,
            type: "Get",
            url: " https://localhost:44314/api/patient/Get?id=" + clientUrl[1],
            success: function (data) {
                $('#id').attr('value', data.id);
                UpdateDetail.PreviousDataRow(data);
            },
            error: function (error) {
                alert(error);
            }
        ...
        $('#btnSave').click(function () {
            UpdateMaster.SavePatient();
        });
    },
    SetDropDownValue: function () {
        var dieaseList = UpdateMaster.GetDisease();
        ...
        var allergieList = UpdateMaster.GetAllergies();
        ...
        var ncdList = UpdateMaster.GetNCD();
    ...
    PreviousDataRow: function (obj) {
        $("#name").val(obj.name);
        $("#diseaseId").val(obj.dieaseId);
        $("#hasEpilepsy").val(obj.hasEpilepsy);

        if (obj.ncdNames.trim() != "" || obj.ncdNames == null) {
            var ncdname = obj.ncdNames;
            var ncds = ncdname.split(",")

            var ncdList = [];

            for (var i = 0; i < ncds.length; i++) {
                $('#select1 option').filter(function () {
                    if ($(this).text() === ncds[i].trim()) {
                        ncdList.push($(this).val());
                    }
                });
            }
            $(`#select1`).val(ncdList);
        ...
        if (obj.allergieNames.trim() != "" || obj.allergieNames == null) {
            var allergiename = obj.allergieNames;// $("#NCDNames").val()
            var allergies = allergiename.split(",")

            var allergieList = [];

            for (var i = 0; i < allergies.length; i++) {
                $('#select3 option').filter(function () {
                    if ($(this).text() === allergies[i].trim()) {
                        debugger;
                        allergieList.push($(this).val());
                    }
                });
            }

            debugger;
            $(`#select3`).val(allergieList);

            $('#select3 option:selected').remove().appendTo('#select4');
        }
    },
